...
/******/ 	__webpack_require__.p = "/zstackplayer/";
/******/
...
var _src = __webpack_require__(329);
...
var options = {
    names: Array.from({ length: 100 }, function (e, i) {
        return "explosion00-frame" + pad(i + 1, 3) + ".png";
    }),
    names2: numstacks ? Array.from({ length: 100 }, function (e, i) {
        return "explosion01-nofire-frame" + pad(i + 1, 3) + ".png";
    }) : null,
    animSpeed: 100,
    initialSlice: 50
};

var player = new _src.ZStackPlayer(options);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__ZStackPlayer_js__ = __webpack_require__(330);
/* harmony reexport (binding) */ __webpack_require__.d(__webpack_exports__, "ZStackPlayer", function() { return __WEBPACK_IMPORTED_MODULE_0__ZStackPlayer_js__["a"]; });
...
<input class="input-range" id="rangeslider" orient="vertical" type="range" step="1" value="0" min="0" max="239">    
...
class ZStackPlayer {
  /**
   * @param {HTMLElement} parentElement the display will try to fill the parent element.
   */
  constructor(options) {
    this.state = {
      images: [],
      ...
      isPlaying: false,
      animationTimer: null,
      animTimePerFrame: 100, // ms
      imageContainer: document.getElementById("imagecontainer"),
      currentFilter: ""
    };

    if (options.animSpeed) {
      this.state.animTimePerFrame = options.animSpeed;
    }

    // TODO handle different lengths of images (min?)
    if (options.names2 && options.names2.length && options.names2.length === options.names.length) {
      this.state.has2 = true;
    }

    for (let i = 0; i < options.names.length; ++i) {
        const img = new Image();
        img.classList.add(slideClass);
        img.src = options.names[i];
        img.style.objectFit = this.state.has2 ? "scale-down" : "contain";
        img.style.height="100%";
        img.style.width= this.state.has2 ? "50%" : "100%";
        this.state.images.push(img);
    }
    if (this.state.has2) {
      for (let i = 0; i < options.names2.length; ++i) {
        const img = new Image();
        img.classList.add(slideClass);
        img.src = options.names2[i];
        img.style.objectFit = "scale-down";
        img.style.height="100%";
        img.style.width="50%";
        this.state.images2.push(img);
      }
    }

    if (options.initialSlice !== undefined) {
      this.state.currentIndex = options.initialSlice;
    }

    this.show(this.state.currentIndex);
    ...
    this.rangeslider.max = options.names.length - 1;
    this.rangeslider.step = 1;
    this.rangeslider.value = this.state.currentIndex;
    this.rangeslider.oninput = this.onRangeSliderChange;

    this.autobtn = document.getElementById("autobtn");
    this.autobtn.onclick = this.applyFilter;
  }
  ...
  show(n) {
    if (this.state.currentImage) {
      this.state.imageContainer.removeChild(this.state.currentImage);
    }
    this.state.currentImage = this.state.images[n];
    this.state.imageContainer.appendChild(this.state.currentImage);

    if (this.state.has2) {
      if (this.state.currentImage2) {
        this.state.imageContainer.removeChild(this.state.currentImage2);
      }
      this.state.currentImage2 = this.state.images2[n];
      this.state.imageContainer.appendChild(this.state.currentImage2);
    }
  }
  ...
  applyFilter() {
    ...
    if (this.state.currentFilter) {
      ...
      this.state.currentFilter = "brightness(50%) contrast(125%)";
    }
    for (let i = 0; i < this.state.images.length; i++) {
      this.state.images[i].style.filter = this.state.currentFilter; 
      if (this.state.has2) {
        this.state.images2[i].style.filter = this.state.currentFilter; 
      }
    }
  }

  incrementSlide(dn) {
    this.state.currentIndex += dn;
  ...
  playNext() {
    this.incrementSlide(1);
    this.rangeslider.value = this.state.currentIndex;
    this.state.animationTimer = setTimeout(this.playNext, this.state.animTimePerFrame);
  }
...
/* harmony export (immutable) */ __webpack_exports__["a"] = ZStackPlayer;
;
...
//# sourceMappingURL=zstackplayer.bundle.js.map